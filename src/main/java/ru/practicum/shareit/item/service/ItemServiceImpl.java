...
public class ItemServiceImpl implements ItemService {
    ...
    public List<ItemDto> searchItems(String text) {
        ...
        return itemRepository.search(text).stream()
                .map(ItemMapper::toDto)  // ← статический вызов
                .collect(Collectors.toList());
    }
    ...
    private ItemDto getItemDtoWithBookingsAndComments(Long userId, Item item) {
        ItemDto itemDto = ItemMapper.toDto(item);  // ← статический вызов
        ...
        if (item.getOwnerId().equals(userId)) {
            ...
            if (lastBooking != null) {
                itemDto.setLastBooking(new ru.practicum.shareit.item.dto.BookingInfoDto(
                        lastBooking.getId(),
            ...
            if (nextBooking != null) {
                itemDto.setNextBooking(new ru.practicum.shareit.item.dto.BookingInfoDto(
                        nextBooking.getId(),
