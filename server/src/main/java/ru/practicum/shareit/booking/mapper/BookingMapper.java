...
import ru.practicum.shareit.user.mapper.UserMapper;// ← ДОБАВИТЬ ЭТОТ ИМПОРТ
...
public class BookingMapper {
    ...
    public static BookingDto toDto(Booking booking, User booker, Item item) {
        ...
        if (booker != null) {
            dto.setBooker(UserMapper.toDto(booker));  // ← теперь работает
        }
        if (item != null) {
            dto.setItem(ItemMapper.toDto(item));      // ← теперь работает
        }
